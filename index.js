...
var appRouter = router({
  ...
  auth: router({
    ...
    verifyOTP: publicProcedure.input(z2.object({ email: z2.string().email(), otp: z2.string().length(6) })).mutation(async ({ input }) => {
...
async function startServer() {
  ...
  server.listen(port, () => {
    const baseUrl = process.env.NODE_ENV === "production" ? "https://osmora.xyz" : `http://localhost:${port}`;
